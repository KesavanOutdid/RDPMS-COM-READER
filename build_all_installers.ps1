...
Write-Host "[1/4] Zipping RDPMS Serial Monitor release build..." -ForegroundColor Yellow
if (Test-Path $RdpmsZip) { Remove-Item $RdpmsZip -Force }
Compress-Archive -Path "$ReleaseBuildDir\*" -DestinationPath $RdpmsZip -Force

Write-Host "[2/4] Zipping APPLICATION FOR SENSEDGE release build..." -ForegroundColor Yellow
...
if (Test-Path $InnoCompiler) {
    Write-Host "[3/4] Compiling RDPMS Serial Monitor Inno Setup installer..." -ForegroundColor Yellow
    $IssScript1 = Join-Path $WorkspaceRoot "serial_monitor\installer.iss"
    & $InnoCompiler $IssScript1

    Write-Host "[4/4] Compiling APPLICATION FOR SENSEDGE Inno Setup installer..." -ForegroundColor Yellow
    $IssScript2 = Join-Path $WorkspaceRoot "serial_monitor\sensedge_installer.iss"
    & $InnoCompiler $IssScript2

    Write-Host "`nAll zips and Inno Setup installers successfully generated in:" -ForegroundColor Green
